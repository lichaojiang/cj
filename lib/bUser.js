...
class user extends tableBase {
    ...
    async init(req) {
        this.role_id = req.body.query.role_id;
        this.user_id = req.body.query.id;
        delete req.body.query.role_id;
    }
    ...
    async setUserRole(req) {
        ...
        if (role_id) {
            ...
            else if (Array.isArray(role_id)){
                for (let role_i of role_id) {
...
class chartData extends tableBase {
    constructor(table) {
        super(table);
        this.promisesOnCreate = [this.setUserId, this.checkDuplicate, this.checkRequired];
        this.promisesOnUpdate = [this.setUserId, this.checkDuplicate];
...
class role extends tableBase {
    ...
    // set variables to class object
    async init(req) {
        this.user_id = req.body.query.user_id;
        this.role_id = req.body.query.id;
        delete req.body.query.user_id;
    }
...
class department extends tableBase {
    constructor(table) {
        super(table);
        this.success_msg = {created: "Department created.", updated: "Department updated", deleted: "Department deleted."};
    }
}
