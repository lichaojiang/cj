...
class user extends tableBase {
    constructor(table) {
        ...
        this.redundant_arr = ['password', 'organization_id'];
...
class role extends tableBase {
    ...
    async appendUserRoleToResults(req, results) {
        ...
        if (sql.getNumOfRows(results) > 0) {
            await Promise.all(results.map(async (result) => {
                let user = await this.getUserRoles(result.id, req.user.organization_id);
                result.user = user;
            }));
        }
    }
    ...
    async getUserRoles(role_id, organization_id) {
        ...
        let queryCmd = `SELECT name FROM ${user_table} WHERE id in (${user_id_cmd})`;

        let users = await this.sqlObj.query(queryCmd);

        let user_arr = [];
        if (sql.getNumOfRows(users) > 0) {
            for (let user_i of users) {
                user_arr.push(user_i.name);
            }
        }
