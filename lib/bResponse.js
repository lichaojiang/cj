...
exports.WARN_NO_DATA = {"returncode": 200, "errcode":-10001, "description":"No data found with current selection."}

exports.WARN_EMPTY_INTERVAL = {"returncode": 200, "errcode": -10101, "description":"Warn: One or more intervals unavailable."}

exports.WARN_CORE = {"returncode": 200, "errcode": -10300, "description":"Warn: Core warnings exist."}
...
exports.ERR_MAX_BUFFER = {"returncode": 400, "errcode": -20004, "description":"Error: Requested data exceeded max buffer."}
exports.ERR_ACCESS_DENIED = {"returncode": 401, "errcode": -20005, "description":"Error: No access right."}

exports.ERR_PLOT = {"returncode": 500, "errcode":-20100, "description":"Error: analysis error."}
...
exports.ERR_SQL_REQUEST_FIELD = {"returncode": 400, "errcode": -20210, "description":"Error: Invalid request field."}
...
exports.send = async (res, data, status=exports.status_OK) => {
    let output = {};
    output.data = data;
    let errcode = status.errcode
    if (errcode !== 0 || typeof errcode === 'undefined')
        errcode = exports.ERROR.errcode;
    let description = status.description || exports.ERROR.description;
    
    output.status = {'errcode': errcode, 'description': description};
    await res.status(status.returncode).send(output);
}
    ...
    if (!isNaN(code.message)) {
        for (let key in exports) {
            if (exports[key].hasOwnProperty('errcode')) {
                if (exports.getErrcode(exports[key]) == code.message) {
                    status = exports[key];
                    break;
                }
            }
...
/*
exports.throw = async (status) => {
    if (status.errcode)
        return await Promise.reject(new Error(status.errcode))
    else
        return await Promise.reject(new Error(exports.ERROR.errcode))
}
*/

exports.throw = async (reject, err) => {
    let status = err;
    if (err.errcode) {
        try {
            throw new Error(err.errcode);
        } catch (error) {
            status = error; // for express error handler
            status.status = err.returncode || 500;
        }
    }

    if (reject)
        return await reject(status);
    else
        return await Promise.reject(status);
}