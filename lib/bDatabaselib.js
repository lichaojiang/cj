...
class tableBase {
    constructor(table){
        ...
        this.promisesFinalRead = [this.extraFilter];
    }
    ...
    async extraFilter(req, results) {
        ...
        for (let key of this.extra_keys) {
            ...
            for (let result_i of results) {
                ...
                if (isPassed)
                    filtered.push(result_i);
            }
        }

        // re-construct results
        results.splice(0, results.length);
        filtered.forEach(result_i => {
    ...
    async read(req) {
        ...
        // final
        await Promise.all(this.getPromisesFinalRead(req, results));
    ...
    async checkDuplicate(req) {
        ...
        await Promise.all(unique_arr.map(async element => {
            if (Object.keys(new_items).includes(element.CONSTRAINT_NAME)) {
                ...
                if (sql.getNumOfRows(row_id) > 0 & await row_id[0].id !== new_items.id) 
    ...
    getPromisesFinalRead(req, results) {
        let promiseList = [];
        for (let i = 0; i < this.promisesFinalRead.length; i++) {
            let promise = this.promisesFinalRead[i];
            let boundPromise = promise.bind(this); // important
            promiseList.push(boundPromise(req, results));
        }
        return promiseList;
