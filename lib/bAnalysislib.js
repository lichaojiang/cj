'use strict'
const bres = require('./bResponse');
const util = require('./bUtils');
const corestatus = require('./bAnalysiscorestatus');

class analysisbase {
    constructor(xlabel, ylabel, title) {
        this.xlabel = xlabel;
        this.ylabel = ylabel;
        this.title = title;
    }

    tojson() {
        return JSON.stringify(this);
    }
}

class line extends analysisbase {
    constructor(x, y, xlabel = "", ylabel = "", title = "", color = "blue") {
        if (Array.isArray(x) === false || Array.isArray(y) === false) throw Error('Expect array');
        // length of x and y should be equal
        if (x.length != y.length) throw Error('x.length != y.length');
        super(xlabel, ylabel, title);
        this.x = x;
        this.y = y;
        this.color = color;
        this.graph_type = "line";
    }
}

class hist extends analysisbase {
    constructor(x_data, y_data, xlabel = "", ylabel = "", title = "", color = "blue") {
        if (Array.isArray(x_data) === false || Array.isArray(y_data) === false) throw Error('Expect array');
        // length of x and y should be equal
        if (x_data.length != y_data.length) throw Error('x_data.length != y_data.length');
        super(xlabel, ylabel, title);
        this.x_data = x_data;
        this.y_data = y_data;

        this.color = color;
        this.graph_type = "hist";
    }
}


exports.analysisLineNoX = (stdout, argv=[]) => {
    let analysis = {};
    let output = util.parseStdout(stdout);

    // setup status
    let status = bres.status_OK;
    if (corestatus.isWARN_NO_DATA(output.status.errcode)) {
        analysis.data = null;
        status = bres.WARN_NO_DATA;
    }
    else {
        // y 
        let y = output.data;
        // x
        let x = Array.from(new Array(y.length), (val, index) => index + 1);

        let xlabel = argv[0] || "";
        let ylabel = argv[1] || "";
        let title = argv[2] || "";

        analysis.data = new line(x, y, xlabel, ylabel, title);
    }

    analysis.status = status

    return analysis;
}

exports.getData = (stdout) => {
    let data = {};
    let output = util.parseStdout(stdout);
    let status = bres.status_OK;
    if (corestatus.isWARN_NO_DATA(output.status.errcode)) {
        data.data = null;
        data.status = bres.WARN_NO_DATA;
    }
    else {
        data.data = output.data
        data.status = status
    }
    return data
}

exports.analysisAny = (stdout, argv=[]) => {
    let output = util.parseStdout(stdout);

    return output;
}

exports.analysisLineWithX = (x, y, xlabel = "", ylabel = "", title = "") => {
    let analysis = {};

    // setup status
    let status = bres.status_OK;
    if (y.length === 0) {
        analysis.data = null;
        status = bres.WARN_NO_DATA;
    }
    else {
        analysis.data = new line(x, y, xlabel, ylabel, title);
    }
    analysis.status = status

    return analysis;
}


exports.analysisHist = (stdout, argv) => {
    let analysis = {};
    let bin;
    if (argv.length >= 1) {
        bin = argv[0];
    }
    else {
        throw Error("Err: Expect argument array length more than 1.");
    }
    
    if(isNaN(bin) || bin < 1) {
        analysis.data = null;
        analysis.status = bres.ERR_BIN_NULL;
        return analysis;
    }

    let output = util.parseStdout(stdout);
    let status = bres.status_OK;
    // setup status
    if (corestatus.isWARN_NO_DATA(output.status.errcode)) {
        analysis.data = null;
        status = bres.WARN_NO_DATA;
    }
    else {
        //y的数据
        let y = output.data;

        var max = y[0];
        var min = y[0];

        // 找最大最小值
        for (var i = 1; i < y.length; i++) {
            if (y[i] >= max) {
                max = y[i];
            }
            if (y[i] <= min) {
                min = y[i]

            }
        }
        // 确保所有数都在区间内
        max += 1;
        min -= 1;


        //x轴的数据
        let x_data = [];
        //区间大小
        let interval = (max - min) / bin;
        for (var i = 0; i < bin; i++) {
            x_data[i] = interval * (i + 1) + min;
        }

        //y轴的数据
        let y_data = [];


        // initialize
        for (let i = 0; i < x_data.length; i++) {
            y_data[i] = 0;
        }
        //循环遍历y数组；判断y数组元素在哪个区间内 就+1,否则进入下一个区间进行判断
        for (let i = 0; i < y.length; i++) {
            //初始化第一个区间
            let range_0 = min;
            let range_1 = x_data[0];
            //初始化y_data的索引
            let range_i = 0;
            while (true) {
                //如果y[i]在对应区间内跳出循环，进入下一次判断
                if (y[i] >= range_0 && y[i] < range_1) {
                    y_data[range_i] += 1;
                    break;
                }
                //进入下一个区间进行判断
                range_0 += interval;
                range_1 += interval;
                range_i++;
            }
        }
        let title=argv[1] || "";
        analysis.data = new hist(x_data, y_data, title);

    }
    analysis.status = status;

    return analysis;
}

exports.line = line;
exports.hist = hist;