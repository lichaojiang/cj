...
class product extends tableBase{
    constructor(sqlObj,table){
        ...
        this.primaryKey = 'id';
        this.required = ['productgroup_id'];
        this.table = table;
    }
    //field_arr: [code, name, productgroup_id]
    create(new_items){
        let sqlObj = this.sqlObj;
        let productObj = this;
        //pre-check promise
        return Promise.all([this.checkDuplicate(new_items),this.checkRequired(new_item)]).then(async() =>{
            let table = productObj.table;
